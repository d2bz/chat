...
type (
	groupMembersModel interface {
		...
		FindByGroudIdAndUserId(ctx context.Context, groupId, userId string) (*GroupMembers, error)
	...
	GroupMembers struct {
		...
		UserId      string         `db:"user_id"`      // 成员用户ID
		RoleLevel   int64          `db:"role_level"`   // 角色等级（1-群主 2-管理员 3-普通成员）
		JoinTime    sql.NullTime   `db:"join_time"`    // 加入时间
		JoinSource  sql.NullInt64  `db:"join_source"`  // 加入来源（1-主动申请 2-邀请 3-扫码）
		InviterUid  sql.NullString `db:"inviter_uid"`  // 邀请人UID
		OperatorUid sql.NullString `db:"operator_uid"` // 操作人UID（审批/邀请人）
	}
)
...
func (m *defaultGroupMembersModel) FindByGroudIdAndUserId(ctx context.Context, groupId, userId string) (*GroupMembers, error) {
