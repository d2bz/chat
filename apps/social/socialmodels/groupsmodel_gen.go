...
type (
	...
	Groups struct {
		Id              string         `db:"id"`               // 群组ID（主键）
		Name            string         `db:"name"`             // 群组名称
		Icon            string         `db:"icon"`             // 群组头像
		Status          sql.NullInt64  `db:"status"`           // 群组状态（1-正常 2-解散 3-封禁）
		CreatorUid      string         `db:"creator_uid"`      // 创建人UID
		GroupType       int64          `db:"group_type"`       // 群组类型（枚举值）
		IsVerify        bool           `db:"is_verify"`        // 是否需要验证加入
		Notification    sql.NullString `db:"notification"`     // 群组公告
		NotificationUid sql.NullString `db:"notification_uid"` // 公告发布人UID
