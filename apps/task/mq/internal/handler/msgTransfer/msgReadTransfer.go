...
const (
	...
	// GroupMsgReadHandlerDelayTransfer 开启消息缓存合并处理，防止短时间内多个用户已读同一条消息导致服务器压力过大，
	// 对已读进行合并，聚合一定数量后再统一处理
...
type MsgReadTransfer struct {
	*baseMsgTransfer
	...
	push      chan *ws.Push            // 传递给聚合器使用，聚合器向通道里发消息即可推送消息
}
...
func (m *MsgReadTransfer) Consume(ctx context.Context, key, value string) error {
	...
	// 下一步要向传回ws向客户端推送消息，把消息转换成Push类型
	push := &ws.Push{
