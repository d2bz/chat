...
type (
	...
	GroupRequests struct {
		...
		ReqId         string         `db:"req_id"`          // 申请唯一标识
		GroupId       string         `db:"group_id"`        // 目标群组ID
		ReqMsg        sql.NullString `db:"req_msg"`         // 申请留言
		ReqTime       sql.NullTime   `db:"req_time"`        // 申请时间
		JoinSource    sql.NullInt64  `db:"join_source"`     // 申请来源（枚举值）
		InviterUserId sql.NullString `db:"inviter_user_id"` // 邀请人UID（邀请场景）
		HandleUserId  sql.NullString `db:"handle_user_id"`  // 处理人UID（群主/管理员）
		HandleTime    sql.NullTime   `db:"handle_time"`     // 处理时间
		HandleResult  sql.NullInt64  `db:"handle_result"`   // 处理结果（1-通过 2-拒绝 0-未处理）
	}
)
...
func (m *defaultGroupRequestsModel) Delete(ctx context.Context, session sqlx.Session, id uint64) error {
